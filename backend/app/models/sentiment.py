...
class ScrapedPost(BaseModel):
    ...
    subreddit: Optional[str] = Field(
        None, description="Subreddit name (for Reddit posts)"
    )
...
class SentimentAnalysis(BaseModel):
    ...
    geopolitical_events: List[str] = Field(
        default_factory=list, description="Geopolitical events mentioned"
    )
    macroeconomic_factors: List[str] = Field(
        default_factory=list, description="Macroeconomic factors mentioned"
    )
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class AnalysisRequest(BaseModel):
    ...
    max_posts: int = Field(
        default=100, description="Maximum number of posts to analyze"
    )
...
class AnalysisResponse(BaseModel):
    ...
    recent_posts: List[ScrapedPost] = Field(
        default_factory=list, description="Recent posts analyzed"
    )
