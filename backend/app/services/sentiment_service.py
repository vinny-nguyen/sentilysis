...
from ..models.sentiment import (
    ScrapedPost, SentimentAnalysis, SentimentSummary, 
    SentimentType, AnalysisRequest, AnalysisResponse
)
...
class SentimentService:
    ...
    async def analyze_ticker(self, request: AnalysisRequest) -> AnalysisResponse:
        ...
        try:
            ...
            summary_data = await gemini_service.generate_summary(mock_posts, request.ticker)
    ...
    async def get_sentiment_history(self, ticker: str, days: int = 7) -> List[SentimentSummary]:
    ...
    async def _get_mock_posts(self, ticker: str, max_posts: int) -> List[Dict[str, Any]]:
    ...
    async def _create_sentiment_summary(
        ...
        average_confidence = sum(s.confidence for s in sentiment_results) / total_posts if total_posts > 0 else 0
