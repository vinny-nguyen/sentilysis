...
from ...models.sentiment import (
    AnalysisRequest, AnalysisResponse, SentimentSummary, 
...
async def get_latest_analysis(ticker: str):
    ...
    try:
        ...
        if not result:
            raise HTTPException(status_code=404, detail=f"No analysis found for ticker {ticker}")
...
async def chat_with_ai(message: ChatMessage):
    ...
    try:
        ...
        context_data = None
        if message.ticker:
            latest_analysis = await sentiment_service.get_latest_analysis(message.ticker)
            if latest_analysis:
                context_data = {
